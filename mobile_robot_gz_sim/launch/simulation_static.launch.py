"""
simulation_static.launch.py

Full navigation simulation in gz sim — the TWO-planner variant: runs both the
regular planner_server and the static_planner_server (the real robot runs two).
Same as simulation.launch.py but uses navigation_launch_gz_static.py and
nav2_params_gz_static.yaml. For the single-planner version use simulation.launch.py.
...
Starts:
  ...
  4. Navigation: navigation_launch_gz_static.py (nav stack + static_planner_server)
  5. RViz2 (optional)

Usage:
  ros2 launch mobile_robot_gz_sim simulation_static.launch.py
  ros2 launch mobile_robot_gz_sim simulation_static.launch.py use_rviz:=False
  ros2 launch mobile_robot_gz_sim simulation_static.launch.py headless:=False
"""
...
def generate_launch_description():
    ...
    declare_params_file = DeclareLaunchArgument(
        'params_file',
        default_value=os.path.join(pkg_gz_sim, 'params', 'nav2_params_gz_static.yaml'),
